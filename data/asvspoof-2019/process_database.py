...
def process_protocol(protocol_path, audio_dir):
    ...
    df["file"] = df["file"].apply(
        lambda x: os.path.join(audio_dir, "flac", f"{x}.flac")
    )
...
def main(data_dir):
    ...
    for name, (folder, proto) in splits.items():
        ...
        try:
            ...
            print(
                f"Warning: Protocol file not found at {proto_path}. Skipping '{name}' split."
            )
            continue

        output_file = os.path.join(f"asv2019_{name}.csv")
    ...
    combined_file = os.path.join("asv2019.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process ASVspoof 2019 LA dataset")
    parser.add_argument(
        "--data_dir",
        type=str,
        default="./LA/",
        help="Path to the 'LA' directory",
    )
