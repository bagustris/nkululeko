...
def main(data_dir):
    ...
    Handles the single-folder layout:
        <data_dir>/ASVspoof2021_DF_eval/flac/            -- audio files
        <data_dir>/ASVspoof2021_DF_eval/trial_metadata.txt  -- ground-truth metadata (preferred)
        <data_dir>/ASVspoof2021_DF_eval/ASVspoof2021.DF.cm.eval.trl.txt  -- ID-only trial list (fallback)

    trial_metadata.txt format (space-separated, no header, 13 columns):
        speaker_id  file_id  codec  channel  system_id  key  trim  subset  ...
    where key is 'bonafide' or 'spoof', mapped to 'real' / 'fake'.
    """

    eval_dir = os.path.join(data_dir, "ASVspoof2021_DF_eval")
    flac_dir = os.path.join(eval_dir, "flac")
    metadata_file = os.path.join(eval_dir, "trial_metadata.txt")
    trial_file = os.path.join(eval_dir, "ASVspoof2021.DF.cm.eval.trl.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to the root directory containing ASVspoof2021_DF_eval/",
    )
